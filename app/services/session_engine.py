...
class SessionEngine:
    ...
    def _prepare_round_if_needed(self, index: int, *, use_llm: bool = True) -> Optional[Dict[str, Any]]:
        """Generate round assets when missing (idempotent)."""
        if index <= 0:
            return None

        sess = self._state()
        prepared_rounds = sess.setdefault("prepared_rounds", {})
        key = str(index)
        existing = prepared_rounds.get(key)
        if existing and not existing.get("error"):
            return existing

        try:
            payload = prepare_round_assets(self.game_state, index, use_llm=use_llm)
            prepared_rounds[key] = payload
            self.game_state.log_event("round_prepared", {"round_index": index})
            ...
            prepared_rounds[key] = error_payload
            self.game_state.log_event("round_preparation_failed", {"round_index": index, "error": str(exc)})
            self.game_state.save()
    ...
    async def confirm_intro(self, *, auto_prepare_round: bool = True, use_llm_rounds: bool = True) -> Dict[str, Any]:
        ...
        if auto_prepare_round:
            next_index = sess.get("round_index", 0) + 1
            prepared_payload = self._prepare_round_if_needed(next_index, use_llm=use_llm_rounds)
    ...
    async def begin_next_round(self, *, auto_prepare_round: bool = True, use_llm_rounds: bool = True) -> Dict[str, Any]:
        """Prépare (optionnellement) et lance la manche suivante après validation de l'intro."""
        sess = self._state()
        next_index = sess.get("round_index", 0) + 1
        ...
        if prepared_payload is not None:
            response["prepared_round"] = prepared_payload
        return response
